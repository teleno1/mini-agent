...
- Shared contracts and lifecycle: `src/mini_agent/tools/contracts.py`, `src/mini_agent/application/ports.py`, `src/mini_agent/domain/sessions.py`, and `src/mini_agent/application/agent.py` define typed Tool/Risk/Call/Result/Registry/Permission boundaries, explicit best-effort cancellation metadata, durable proposed/validated/started/completed/failed/interrupted states, redacted permission records, deeply immutable normalized-call authorization, normalized resource summaries, final path/argument-hash rechecks, active-time budgeting, and no Tool-level UI or prompting.
